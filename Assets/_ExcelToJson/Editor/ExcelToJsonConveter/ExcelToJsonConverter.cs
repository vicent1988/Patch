...
public class ExcelToJsonConverter
{
    ...
    public void ConvertExcelFilesToJson(string inputPath, string outputPath, bool recentlyModifiedOnly = false)
    {
        ...
        if (succeeded)
        {
            ...
            Debug.Log("<color=yellow>Excel to Json Completed!!</color>");
    ...
    public bool ConvertExcelFileToJson(string filePath, string outputPath)
    {
        ...
        {
            if (excelData.Tables[i].TableName.Contains("Sheet"))
                continue;
            ...
            else
            {
                ...
                //if (listServer.Contains(fileName))
                //{
                //    WriteTextToFile(spreadSheetJson, outputPath + "/_server/" + fileName + ".json");
                //}
    ...
    private DataSet GetExcelDataSet(string filePath)
    {
        ...
        catch (Exception e)
        {
            ...
            Debug.LogError(filePath + "  " + e.ToString());
            throw new Exception();
        }
        finally
        {
...
#if UNITY_EDITOR
        PLogger.Log("spreadsheet name:" + excelReader.Name);
#endif
    ...
    {
        QM.gd.SaveDB_to_file();

        if (UnityEditor.EditorUtility.DisplayDialog("提示", "所有数据都转换完成!", "确认"))
        {
            EditorMode.SetAssetBundleNameCustom("Data");
        }
    }
